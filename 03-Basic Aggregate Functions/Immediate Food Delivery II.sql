...
ImmediateFirstOrders AS (SELECT d.customer_id FROM Delivery d
...
SELECT ROUND((COUNT(DISTINCT i.customer_id) * 100.0) / (SELECT COUNT(DISTINCT customer_id) FROM FirstOrders), 2 ) AS immediate_percentage
FROM ImmediateFirstOrders i;
Input: 
Delivery table:
+-------------+-------------+------------+-----------------------------+
